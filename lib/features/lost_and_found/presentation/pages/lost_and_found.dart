...
class _LostAndFoundState extends State<LostAndFound> {
  ...
  List<LostAndFoundItemModel> filteredItems = [];
  ...
  void _onSearchChanged() {
    ...
    setState(() {
      filteredItems = lostFoundItems.where((item) => item.name.toLowerCase().contains(query)).toList();
    });
  }
  ...
  void _navigateToAddScreen() async {
    ...
    if (result != null && result is LostAndFoundItemModel) {
      setState(() {
        lostFoundItems.add(result);
        _onSearchChanged(); // update filtered list after adding
      });
    }
  }
